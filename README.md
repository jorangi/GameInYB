...
<details>
  ...
  - 장비 착용 관련 버그 수정
    ...
    ## v.2(신종혁)
    - 치명타 추가 (공격력 * 치명타피해량 - 방어력)
    - 플레이어의 공격 관련 시각, 음향 효과
      - 몬스터와 플레이어의 피격음 추가
      - 몬스터가 피격시 피격 이펙트가 생성됨
        - 치명타시 다른 이펙트
    - 피격시 시각 효과 추가
      - 카메라 흔들림
        - 피해량에 따라 카메라 흔들림 정도가 달라짐
      - 피격 순간 화면에 붉은 테두리 발생
    - 현재 체력에 따라 화면에 붉은 테두리 정도가 달라짐
      - 체력이 50% 미만일경우 발생
      - 체력이 낮을수록 더욱 진해짐
    - 오브젝트 풀링을 이용하여 최적화
      - 플레이어 공격 히트박스
    ...
    ## v.1(신종혁)
    - 몬스터의 공격 위치 문제 수정
      - 몬스터의 공격 히트박스가 몬스터의 몸체에서 생성되는 버그
      - 몬스터의 공격 방향과 히트 박스의 방향이 일치하지 않는 버그
    - 몬스터 피격 음향 조절
      - 음량을 20프로 증가(wav파일을 수정)
    - 피격 이펙트(Hit Spark) 사이즈 조정 (Pixel Per Unit: 100 -> 16)
    - 히트 스파크 위치 조정
      - 피격자 위치 중심 난수 * 0.5
    - 히트 스파크가 1개만 생성되던 문제
      - 히트 스파크의 Sibling 변경 로직을 Enable 라이프 사이클로 이동
    - 인게임에서 옵션 패널이 보이지 않던 문제 해결
    ## v.2(신종혁)
    - 무기 가죽 글러브 추가
      - 양손(보조 공격)무기
    - 플레이어의 무기 공격 애니메이션 로직 변경
      - 플레이어의 무기 공격 애니메이션을 애니메이션 이름을 통해 재생
      - 무기 스킬을 통해 애니메이션을 적용

    # 251110
    ## v.1(신종혁)
    - 무기 추가
      - 나무 단검, 철 단검, 나무 몽둥이, 철 창
    - 찌르기 이펙트 추가
    - 데미지 숫자 추가(sprite 기반)
    - TakeDamage 함수 인자 수정
    ...
    - 인벤토리 모달창 조정
      - 크기 조정 (200~350)
      - 위치 조정 (슬롯보다 절반정도 아래)
      - 링크 텍스트 클릭시 부가정보 모달 표시(최대 3개)
      - 아이템 정보를 보다 세밀하게
        - 등급, 보유하는 스킬
    - 스킬 데이터 구조, 로더 생성(ServiceHub에 스킬 레포지토리 등록)
</details>