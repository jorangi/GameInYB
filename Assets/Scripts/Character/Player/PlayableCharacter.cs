...
public interface IInventoryData
{
    ...
    PlayerEquipments Equipment { get; }
...
public class PlayableCharacter : Character, IInventoryData, IInventorySnapshotProvider
{
    ...
    public PlayerEquipments Equipment => inventory.equipments;
    public GameObject CharacterInformationObj;
    ...
    [SerializeField] private Weapon weaponScript; // 플레이어의 메인무기 스크립트
    [SerializeField] private Weapon subWeaponScript; // 플레이어의 보조무기 스크립트
    ...
    public const float walkDustMaxTimer = 0.2f;
    [SerializeField] public float walkDustTimer = 0.19999f;
    ...
    protected override void Awake()
    {
        ...
        subWeaponScript = col.GetComponentInChildren<Weapon>();
        ...
        //SetupMessageBox();
        DustSetup();
        dmgPrefab = ServiceHub.Get<IAddressablesService>().GetPrefab("FloatingDamage");
    }
    ...
    {
        if (ServiceHub.isLoadedFromLogin)
        {
            InitPos();
            return;
        }
    ...
    /// </summary>
    /// <returns></returns>
    private async UniTask<int> InitAtlas()
    {
    ...
    void OnEnable()
    {
        ...
        inputAction.Player.SubWeapon.performed += OnSubWeapon;
    ...
    private void OnSubWeapon(InputAction.CallbackContext context)
    {
        SubWeaponAttack();
    }
    /// <summary>
    ...
    {
        subWeaponScript.StartAttack();
    }
    float dropDownTimer = 10;
    float landSFXTimer = 0f;
    float vibratingTimer = 0f;
    protected override void Update()
    {
        // Character(부모 클래스)의 Update 메소드 호출
        base.Update();
        if (vibratingTimer > 0.0f)
            vibratingTimer -= Time.deltaTime;
        else if(vibratingTimer < 0.0f)
        {
            vibratingTimer = 0.0f;
            FindAnyObjectByType<CinemachineBasicMultiChannelPerlin>().AmplitudeGain = 0;
        }
        if (!lateGroundCheck && isGround && landSFXTimer <= 0.0f)
        {
            AudioManager.Inst.PlaySFX("land");
            lateGroundCheck = true;
            landSFXTimer = 0.05f;
        }
    ...
    protected override void FixedUpdate()
    {
        ...
        if (weaponScript.anim.GetBool("IsAttack")) return;
        #region legacy_code
        ...
        // Transform mainWeaponPivot = weaponScript.transform.parent;
        // Debug.Log($"angleDeg: {angleDeg} / pivotRot: {mainWeaponPivot.eulerAngles.z}");
        // arm.localScale = new(((angleDeg < -90 && angleDeg >= -180) || (angleDeg < 180 && angleDeg > 90)) ? -1 : 1, 1, 1);
        // if (angleDeg >= -180 && angleDeg <= 0) //상
        // {
        //     mainWeaponPivot.transform.SetLocalPositionAndRotation(new(0.022f, 0.165f), Quaternion.Euler(0, 0, -154.777f));
        // }
        // else if (angleDeg > 0 && angleDeg < 180) //하
        // {
        //     mainWeaponPivot.transform.SetLocalPositionAndRotation(new(0.408f, -0.155f), Quaternion.Euler(0, 0, 48.392f));
        // }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("FallingZone"))
        {
            ...
            TakeDamage(data.MaxHP * 0.2f, false, true);
        }
    }
    ...
    public override void TakeDamage(float damage, bool isCritical = false, bool isFallingDmg = false) // 피해 적용 로직
    {
        base.TakeDamage(damage);
        if (damage < 0.0f) return;
        if (isGuard && !isFallingDmg)
        {
            GameObject dObj = Instantiate(dmgPrefab, transform.position + new Vector3(UnityEngine.Random.Range(-1f, 1f), 2f, 0), Quaternion.identity);
            dObj.GetComponent<TextMeshPro>().text = "<sprite name=\"guard\">";
            AudioManager.Inst.PlaySFX("player_hit");
        }
        else
        {
            ...
            FindAnyObjectByType<CinemachineBasicMultiChannelPerlin>().AmplitudeGain = (float)dmg / Data.MaxHP;
            vibratingTimer = 0.1f;
            GameObject dObj = Instantiate(dmgPrefab, transform.position + new Vector3(UnityEngine.Random.Range(-1f, 1f), 2f, 0), Quaternion.identity);
            StringBuilder sb = new();
            foreach (char c in Mathf.RoundToInt(dmg).ToString())
                sb.Append($"<sprite name=\"hitdmg_{c}\">");
    ...
    public void SetMainWeapon(Item item)
    {
        ...
        weaponScript.anim.SetBool("IsAttack", false);
        weaponScript.attackIdx = 0;
        OnEquipmentChanged?.Invoke(EquipmentType.MAINWEAPON, item);
        weaponScript.attackQueue.Clear();
        foreach (string s in item.skills)
        {
            if (s[1] == '1') weaponScript.attackQueue.Add(int.Parse(s));
        }
    ...
    {
        subWeaponSprite.color = isTwoHander && inventory.equipments.MainWeapon.item.id != "01010" ? new(1, 1, 1, 0) : Color.white;
        subWeaponSprite.sprite = ServiceHub.Get<IAtlasService>().GetSprite("Weapons", inventory.equipments.MainWeapon.item.id != "01010" ? item.id : "01010");
        Data.GetStats().RemoveProvider(item.GetProvider());
        inventory.equipments.SubWeapon.item = item;
        subWeaponScript.anim.SetBool("IsAttack", false);
        subWeaponScript.attackIdx = 0;
        if (!isTwoHander)
        {
            Data.GetStats().AddProvider(item.GetProvider());
            subWeaponScript.attackQueue.Clear(); //보조무기 공격 큐(리스트) 초기화
            subWeaponScript.weaponSkill = (() => { }); //보조무기 스킬 비움
            foreach (string s in item.skills)
            {
                if (s[1] == '1')
                {
                    subWeaponScript.attackQueue.Add(int.Parse(s));
                    subWeaponScript.weaponSkill += (() =>
                    {
                        if (s == "21002") // 가드
                        {
        ...
        if (inventory.equipments.MainWeapon.item.id == "01010")
        {
            subWeaponScript.attackQueue.Clear();
            foreach (string s in inventory.equipments.MainWeapon.item.skills)
            {
                if (s[1] == '1') subWeaponScript.attackQueue.Add(int.Parse(s));
            }
            Data.GetStats().ForceRecalulate();
        }
    ...
    {
        Backpack[itemslot.index].item = default;
        InvokeBackpackChanged(itemslot.index, Backpack[itemslot.index]);
    }
    /// <summary>
    /// 백팩 초기화
    /// </summary>
